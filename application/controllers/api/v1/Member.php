        ...
        try{
         $config = [
                ...
                if($this->form_validation->run()==FALSE){
                    ...
                    $confirm_password_error = $this->form_validation->error('password_confirmation');  
        ...
        try{
         $config = [
                        ...
                        [
                            'field' => 'password_confirmation',
                            'rules' => 'required|trim|matches[password]',
                            'errors' => ['required' => 'Password and Confirm Password does not match'],
                        ],
                    ...
                    if (!empty($gender_error)){
                        ...
                        if($lastuserid){							
							...
							curl_setopt_array($curl, array(
							  CURLOPT_URL => 'https://api-staging.cinch.io/api/v1/members',
							...
							if (array_key_exists("error",$data)){					
								$status = parent::HTTP_OK;
								$this->return["status"]=0;								
								$this->return["message"]=$data['error'];					
								$this->response($this->return, $status);
							}else{																

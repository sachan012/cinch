        ...
        <div class="card-body">
           ...
           <!-- For Messages -->
            ...
            <?php echo form_open(base_url('admin/users/edit/'.$user['id']), 'class="form-horizontal"' )?> 
              ...
              <div class="form-group">
                <label for="assigned_station" class="col-md-2 control-label"><?= ucwords('assigned station') ?></label>

                <!-- <div class="col-md-12">
                  <input type="text" name="assigned_station" value="<?= $user['assigned_station']; ?>" class="form-control" id="assigned_station" >
                </div> -->
                <select class="form-control" name="assigned_station" class="form-control">
                   <option value="">Select Station from the list</option>
                    <?php foreach($stations as $st){ ?>                   
                      <option value="<?php echo trim($st['station_code']);?>" <?php if($user['assigned_station']==trim($st['station_code'])){echo "selected";}?>><?php echo $st['station_name'].'('.trim($st['station_code']).')';?></option>
                    <?php } ?> 
                    </select>
              </div>
              <div class="form-group">
